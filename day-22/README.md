...
Esse padrão só deve ser considerado viável quando as aplicações envolvidas adotam modelos de [comunicação assíncrona](https://fidelissauro.dev/mensageria-eventos-streaming/) e operam com [consistência eventual](https://fidelissauro.dev/teorema-cap/).
...
Essa abordagem **permite que testes sejam executados na nova versão sem impactar o fluxo produtivo, tornando possível a execução de warm-ups das réplicas**, a realização de [smoke tests](https://fidelissauro.dev/load-testing/) e a aplicação de validações automáticas ou manuais antes da promoção da versão, garantindo maior segurança no processo.

Quando a nova versão “Green” está pronta e testada, o tráfego de produção é redirecionado do ambiente Blue para o Green. Esse redirecionamento geralmente ocorre de forma instantânea, utilizando um [Load Balancer](https://fidelissauro.dev/load-balancing/) ou um mecanismo de [Roteamento de DNS](https://fidelissauro.dev/protocolos-de-rede/).
